    ...
    <div class="bg-amber-50 min-h-screen py-12">
      <div class="container mx-auto px-4">
        ...
        <div class="text-center mb-12">
          ...
          <p class="mt-4 text-stone-600">Immerse yourself in divine knowledge and music.</p>
        </div>
        ...
        <div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
             @for (item of templeService.library(); track item.id) {
              @if (item.type === 'ebook') {
                <div class="bg-white rounded-lg shadow p-6 border border-amber-100 hover:shadow-lg transition-shadow">
                  <div class="h-40 bg-stone-100 rounded mb-4 flex items-center justify-center text-stone-400">
                  ...
                  <a [href]="item.url" target="_blank" class="block text-center w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 rounded transition-colors">
                    Download PDF
                  </a>
                </div>
              }

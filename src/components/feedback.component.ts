            ...
            @if (!submitted()) {
              <form (submit)="handleSubmit($event)">
                <div class="mb-4">
                  ...
                  <input [(ngModel)]="name" name="name" required class="w-full px-4 py-2 border border-stone-300 rounded focus:border-amber-500 focus:outline-none" placeholder="Enter Name">
                </div>
                
                <div class="mb-6">
                   <label class="block text-stone-700 font-bold mb-2">Feedback / Suggestion</label>
                   <textarea [(ngModel)]="message" name="message" required class="w-full px-4 py-2 border border-stone-300 rounded h-32 resize-none focus:border-amber-500 focus:outline-none" placeholder="Share your experience..."></textarea>
                </div>

                <button type="submit" class="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 rounded transition-colors shadow">
                  Submit Feedback
                </button>
              </form>
            } @else {

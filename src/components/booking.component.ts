            ...
            @if (templeService.siteConfig().enableBooking) {
                              ...
                              <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3">
                                 @for (slot of slots; track slot.time) {
                                    <button (click)="selectSlot(slot)" [disabled]="slot.status === 'FULL'" 
                                       ...
                                       @if (slot.status !== 'FULL') {
                                           <div class="text-[9px] mt-1 bg-black/20 px-1 rounded">{{ slot.capacity - slot.booked }} Seats</div>
                                       } @else {
                                           <div class="text-[9px] mt-1 bg-black/20 px-1 rounded">FULL</div>
                                       }
                                    </button>
                                 }
                              </div>
               ...
               <div class="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
                  <div class="w-24 h-24 bg-stone-100 rounded-full flex items-center justify-center mb-6">
                     <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-12 h-12 text-stone-400"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
                  </div>
                  <h3 class="text-2xl font-bold text-stone-700 font-serif">Online Booking Paused</h3>
                  <p class="text-stone-500 max-w-md mt-2">Darshan booking services are temporarily unavailable. Please visit the temple counter for tickets or check back later.</p>
               </div>
            }
          </div>
        </div>
